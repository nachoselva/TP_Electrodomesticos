...
public class JfrmLista{
	...
	private boolean verificarFormato()
	{
		...
		try
		{
			if(!this.txtPrecioMax.getText().isEmpty())
			{
				Float.parseFloat(this.txtPrecioMax.getText());
			}
			if(!this.txtPrecioMin.getText().isEmpty())
			{
				Float.parseFloat(this.txtPrecioMin.getText());
			}
			bandera = true;
		}
		catch (Exception e)
		{
			bandera = false;
			JOptionPane.showMessageDialog(this.frmElectrodomesticos, "Verificar Formatos de Entrada", "ERROR", 2 );
		}
		return bandera;
	}
	
	private void filtrar_completo()
	{
		if(verificarFormato())
		{
		recargarDatos();
